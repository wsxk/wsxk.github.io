...
**unsorted bin放入的第一个chunk的fd和bk指针通常是main_arena+96,其本身存放的是top_chunk地址**<br>

## 2.5 largebin<br>
```
1. Doubly linked lists(双向链表)
2. Bins consist of a range of sizes（一个bin存放的是一个范围chunk,这给维护带来了额外挑战）
3. Chunks in each bin are sorted by size,  largest first
largebin有一个链表头x，x->fd指向最大size的chunk，x->bk指向最小size的chunk
各个chunk中，fd_nextsize/bk_nextsize指向其他size的chunk，fd_nextsize指向更小的chunk，bk_nextsize指向更大的chunk
