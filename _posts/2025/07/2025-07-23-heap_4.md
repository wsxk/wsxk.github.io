...
- [4. fastbin attack](#4-fastbin-attack)
  - [4.1 fastbin poison](#41-fastbin-poison)
  - [4.2 fastbin double free -\> tcachebin](#42-fastbin-double-free---tcachebin)
- [5. 综合利用](#5-综合利用)
  - [5.1 案例一: fastbin attack + unlink](#51-案例一-fastbin-attack--unlink)
  - [5.2 案例二：tcache泄露地址+ off\_byte\_null+unlink](#52-案例二tcache泄露地址-off_byte_nullunlink)
- [6. 写在最后](#6-写在最后)
...
# 4. fastbin attack<br>
## 4.1 fastbin poison<br>
fastbin和tcachebin类似，而且他的poison操作非常容易实现:<br>
```python
...
## 4.2 fastbin double free -> tcachebin<br>
使用场景:<br>
```
1. 只使用malloc分配
2. 只有double free漏洞
...
# 5. 综合利用<br>
了解、掌握了各个攻击的单点攻击手法，还是不够的，现在的ctf（包括现网环境），往往单点的攻击手法已经无法突破已有的防御手段了（或只达成了部分目的），所以，综合利用多个漏洞/攻击手法是现在的解决方案。<br>
## 5.1 案例一: fastbin attack + unlink<br>
上文提到：`unlink的使用场景通常是一个已知的地址中存放指向chunk的指针，且这个chunk是可以被unlink的`<br>
结合`fastbin`攻击的话，我们可以先在fastbin中伪造一个假chunk，利用`fastbin poison`获得该chunk，继而修改下一个chunk的大小，实现unlink。<br>
**unlink时，并不关注上一个chunk的大小,另外要注意，我们只需要consolidate backward，不需要forward**<br>
```python
...
p.interactive()
```
## 5.2 案例二：tcache泄露地址+ off_byte_null+unlink<br>
```python
...
# 6. 写在最后<br>
写poc出现bug，或者遇到一些难题，或者你想要看看当前的利用方式，可以参考如下几个链接:<br>
[https://elixir.bootlin.com/glibc/glibc-2.35/source/malloc/malloc.c](https://elixir.bootlin.com/glibc/glibc-2.35/source/malloc/malloc.c)<br>
[https://github.com/shellphish/how2heap/tree/master/glibc_2.35](https://github.com/shellphish/how2heap/tree/master/glibc_2.35)<br>
