...
**tips 1 ：利用前可以事先分配内存A和B（同样大小），再这个基础上做内存的释放和再分配，此时泄露的地址就全部都是稳定的了！（A和B的地址（pthread heap一般有效位为48位）高24位一致，借此能够进行地址的de-mangling）**<br>
**tips 2 : 可以尝试使用gdb的命令 find start_addr,end_addr,value搜索内存段中的值**<br>
