...
- [5. AI agent开发框架](#5-ai-agent开发框架)
  - [5.1 AutoGen](#51-autogen)
  - [5.2 AgentScope](#52-agentscope)
...
```
## 5.1 AutoGen<br>
AutoGen 的设计哲学根植于"以对话驱动协作"。它巧妙地将复杂的任务解决流程，映射为不同角色的智能体之间的一系列自动化对话。<br>
autogen其实也很吃`prompt`的设计，如果你prompt没设计好的话，大概率各个角色工作就不稳定。<br>
而且AutoGen很看模型怎么样，即使是与`openai`兼容的格式，也不一定能很好的兼容其代码（例如，perplexity的sonar模型在这里就不适用。<br>
代码链接: [https://github.com/datawhalechina/hello-agents/blob/main/code/chapter6/AutoGenDemo/autogen_software_team.py](https://github.com/datawhalechina/hello-agents/blob/main/code/chapter6/AutoGenDemo/autogen_software_team.py)<br>

AutoGen的优缺点:<br>
```
优点：
1. 门槛低： 无需为智能体设计复杂的状态机或控制流，减低复杂任务建模门槛
2. 可复用： 通过系统消息为每个智能体赋予高度专业化的角色，精心设计的智能体可被复用。
3. 协作流程可预测： 能规定智能体的发言顺序来决定协作流程。

缺点：
1. 基于LLM对话，容易偏离预期
2. 调试困难，答案不合预期时，吐出的不是调用栈，而是对话记录。“对话式调试“
3. 非OpenAI模型需要补充模型信息。而且对某些模型支持不友好，比如perplexity ai
```


## 5.2 AgentScope<br>
如果说 AutoGen 的设计哲学是"以对话驱动协作"，那么 AgentScope 则代表了另一种技术路径：**工程化优先的多智能体平台**。<br>
其架构设计图如下所示:<br>
![](https://raw.githubusercontent.com/wsxk/wsxk_pictures/main/2025-9-25/20260121235207.png)


# references<br>
学着学着学歪了。或者说学习的思路有问题,我们是为了解决问题而学习技术，而不是单纯为了学习。`datawhale`虽然很好很全，但是不适合用于初学者一步一步学习（看多了就腻了），后续如果需要开发agent（记忆管理，我或许会重新翻一下对应的章节吧。）<br>
...
[世界模型](https://github.com/knightnemo/Awesome-World-Models)<br>
